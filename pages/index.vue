<template>
    <div class="p-8 bg-gradient-to-r from-pink-300 to-blue-200 min-h-screen flex flex-col justify-center items-center">
      ...
      <button ref="loadMoreBtn" @click="loadMore" class="mb-8 mt-4 px-6 py-3 bg-pink-500 hover:bg-pink-600 text-white rounded-md text-xl font-semibold shadow-md">
        Load More
      </button>
    </div>
  </template>
  ...
  const loadMore = async () => {
    await catStore.fetchCatImages(catImages.value.length / 10 + 1, 10, breed.value); // Assuming each page contains 10 images
    // Scroll to the "Load More" button after images have been loaded
    
    if (loadMoreBtn.value) {
        loadMoreBtn.value.scrollIntoView({
        behavior: 'smooth',
        block: 'end', // Scroll to the bottom of the button
      });
    }
};
  </script>
